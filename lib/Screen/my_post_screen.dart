            ...
            return Center(
              child: Padding(
                ...
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Icon(Icons.feed_outlined, color: Colors.grey[400], size: 80),
                    const SizedBox(height: 15),
                    Text(
...
class PostCard extends StatelessWidget {
                ...
                Align(
                  ...
                  child: PopupMenuButton<String>(
                      ...
                      if (value == 'edit') {
                        ...
                        final confirm = await showDialog<bool>(
                          ...
                          builder: (context) => AlertDialog(
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
                            title: const Text('Xác nhận xóa bài viết', style: TextStyle(fontWeight: FontWeight.bold)),
                            content: const Text(
                            ...
                            SnackBar(
                              content: const Row(
                                children: [
                                  ...
                                  SizedBox(width: 10),
                                  Text('Đang xóa bài viết...', style: TextStyle(color: Colors.white)),
                                ],
                              ),
                        ...
                        String newStatus = (current == 'san_sang' || current == 'sẵn sàng') ? 'dang_giao_dich' : 'san_sang';

                        final confirmChange = await showDialog<bool>(
                          context: context,
                          builder: (context) => AlertDialog(
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
                            title: const Text('Xác nhận đổi trạng thái', style: TextStyle(fontWeight: FontWeight.bold)),
                            content: Text(
                        ...
                        if (confirmChange == true) {
                            ...
                            SnackBar(
                              content: const Row(
                                children: [
                                  CircularProgressIndicator(color: Colors.white, strokeWidth: 2),
                                  SizedBox(width: 10),
                                  Text('Đang đổi trạng thái...', style: TextStyle(color: Colors.white)),
                                ],
                              ),
                              backgroundColor: Colors.blueAccent,
                              behavior: SnackBarBehavior.floating,
                          ...
                          final success = await doiTrangThaiBaiDang(post.id, newStatus);
                          if (context.mounted) {
                             ScaffoldMessenger.of(context).hideCurrentSnackBar();
                            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                              content: Row(
                    ...
                    itemBuilder: (_) => <PopupMenuEntry<String>>[
                      PopupMenuItem(
                        value: 'edit',
                        child: _buildPopupMenuItemContent(Icons.edit, 'Chỉnh sửa'),
                      ),
                      PopupMenuItem(
                        value: 'change_status',
                        child: _buildPopupMenuItemContent(
                            Icons.published_with_changes,
                            (post.trangThai.toLowerCase() == 'san_sang' || post.trangThai.toLowerCase() == 'sẵn sàng')
                      ...
                      PopupMenuItem(
                        value: 'delete',
                        child: _buildPopupMenuItemContent(Icons.delete_outline, 'Xóa bài viết', textColor: Colors.red),
                      ),
                    ],
                  ),
                ),
  ...
  Widget _buildPopupMenuItemContent(IconData icon, String text, {Color? textColor}) {
    return Row(
      children: [
        Icon(icon, color: textColor ?? Colors.grey[700]),
        const SizedBox(width: 10),
        Text(text, style: TextStyle(color: textColor ?? Colors.grey[800], fontSize: 15)),
      ],
    );
  }
